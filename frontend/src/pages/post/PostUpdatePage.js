...
const PostUpdatePage = ({ match }) => {
  ...
  const sendPostData = () => {
    ...
    axios.put('/post/'+`${match.params.userId}`, postData, config)
      .then(() => {
  ...
  const deleteData = () => {
    axios.delete('/post/'+`${match.params.userId}`,config)
      .then((res) => {
        console.log(res)
      })
      .catch((error) => {
    ...
    <div style={{display:"flex", justifyContent:"center", marginTop:"25px"}}>
      <div className="post-create-box">
        ...
        <div style={{display:"flex", justifyContent:"space-around"}}>
          <button className="post-create-btn" onClick={sendPostData}>삭제하기</button>
          <button className="post-create-btn" onClick={sendPostData}>수정하기</button>
        </div>
      </div>
    </div>
  )
